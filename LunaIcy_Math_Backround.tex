...
\begin{center}
    ...
    \begin{minipage}{0.8\textwidth}
        ...
        \small 
        \noindent 
        This little report explores \cite{https://doi.org/10.48550/arxiv.2410.15844} the sintering of ice grains in the ice sheet of Jupiters moon Europa. It approaches the problem from a mathematical point of view and attempts to reimpliments the scheme. In this paper the sintering was modeled via a system of ODEs that where coupled with a heat equation. The difference to the original paper is that here the numerical scheme uses implicit Euler to solve the ODE and by doing so, avoids the limitation of the small step size that occure due to the stiffnes of the problem. Second, a solver for the heat equation is here derived by hand. It handles the Neuman boundary data by incorporating ghost cells and solves the PDE with the method of lines. The resulting system of ODEs is again due to stiffness solved via implicit Euler. Note that due to the fact that in the given PDE there is no standard Lapalce operator a three point stencil cannot be derived using finite differences dirctly. Using finite volumes combined with finite differences however yields a three point method. Finally, in a fun little excersize the geometry of the ice grains was derived via manifold integrals, but not used in the implimation as their solution was more complex compared to the simple analytic solution.
        \vspace{0.3cm} 
    \end{minipage}
\end{center}

\textbf{Motivation:}
In light of the upcoming missions planned to Jupiters moon Europa such as JUICE and even mission that will attempt to drill through the ice sheet, the geometry of the ice grains are of great interest in order to succefully navigate such a mission! Such a model for the ice sheet does not only depend on the sintering, but also on external influences such as temperature, radiation and micro meteor impacts. A deterministic model was proposed in \cite{https://doi.org/10.48550/arxiv.2410.15844} that couples the sintering and temperature. Future work would extend this deterministic model to a stochastic one that includes micro meteor impacts. In order to make worst case bounds on the microstructure of such a stochastic model one would need to rely on extreme value theory for stochastic processes. This is briefly motivated in the Outlook. The rest of this report focuses on the reimplementation of the deterministic model.




Consider a one dimensional slice of the Ice sheet of Jupiters moon Europa. This is our state space $\Omega := [0,d] \subset \mathbb{R}$ where $d$ denotes the depth od the ice sheet. We now try to model the changing ice grains and their bonds to their neighboring grains such that a steady state (fixed point of the dynamic system) is reached that aims to explain the ice grain and bond size in the ice sheet of Europa. In \cite{https://doi.org/10.48550/arxiv.2410.15844} it was proposed that the low thermal inertia of the porous ice can give way for us to presume that the depth of significant tenperature changes before dissipation is very low (only a few centimeters). Thus in the following study the ice sheet can be limited to only a small depth. Further, due the low thermal inertia the bottom for the to be defined heat equation the boundary data should ensure that the temperature change has vanished, when reaching the bottom.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{Pictures/Overview.png}
  \caption{Overview of the discretization space $\Omega_h$ and the microsstructure at each grid point and temperature that is coupled.}
  \label{Fig: Overview}
\end{figure}
...
\begin{figure}[H]
  ...
  \label{Fig: Geometry}
\end{figure}
This change of radii/microstructure is modeled indirectly via the water vapor transport. Consider the conservation law (i.e. we have a closed system that does not loose or gain mass) 
\[
...
A benifit of the approach of modelling the masses is that we do not make assumptions on the underlying intial water vapor pressure, but only on its initial mass. Using the the Hertz-Knudsen formula mass transport only occurs proportionally to the difference in gas pressure in the pore space and the saturated water vapor pressure over a flat surface. It is defined for every $x \in \Omega_h$ as
\begin{align}
...
\end{align}
which is an asymptotic stable linear ODE (in section 2.1 it can be seen that the flux $J_i$ is linear in $m_{gas}$ and has negative derivative). Here $S_{g}$ denotes the area of the grain in contact with the outward gas and $J_{g}$ is the repective flux, $ S_{b}(t)$ the area of the bond in contact with the gas and $J_b$ is the respective flux. Only if these fluxes are non zero sublimation will occur. We assume that this fixed point is reached instantanuously, i.e. we assume $ \frac{\partial}{\partial t} m_{gas}(x,t)=0$. Then grain/bond gains/losses of mass is described according to 
\[
...
\end{pmatrix} ,
\end{equation}
where $v_g$ are calculated using Equation (\ref{Eq: vg}) and (\ref{Eq: vb}). As seen in Figure \ref{Fig: Geometry} the point (actually circle) $\hat{x^*}$ denotes the point/area where the two surface areas of the grain and bond intersect. Thus, while optimizing for $r_g$ and $r_g$ the above function also needs to calculate $\hat{x}^*$ for every radii that is suggested by the minimization problem. This is briefly touched upon in Section 5.4. Note that Equation ()\ref{Eq: f}) is a non-trivial optimization problem that will not be further discussed. \\
...
Here the Physics will be given a little overview. As I am not a phsisist, this is only a broad overview.
...
\]
This last term is here links the PDE to the ODE that describes sintering.
...
To numerically solve the PDE (\ref{PDE}) we first discretize the state space by $\Omega_h \subset \Omega$ and use the method of lines approach. We integrate both sides from $[x_{i-\frac{1}{2}}, x_{i+ \frac{1}{2}}]$ and divide by the length of the intervall $h$: 
\begin{align*}
...
\]
This approximation only holds for the interior of our domain $\Omega$. In order to include the boundary data into the solution we need to handle the Neumann boundary data (\ref{Neumann}) on both sides. This is doen by introducing ghost cells. We aproximate the boundary data by symmetric differences, i.e. also second order, we get 
\begin{align*}
...
\]
and on that proposed intervall $\sin (\vartheta)$ is monotonically decreasing. In total we get for $r < r_g$:
\begin{align*}
...
\end{align*}
The curvature of the grain is $K_g = \frac{2}{r_g}$ and of the bond $K_b =- \frac{1}{r_b}$ repectively. The difference of sign is due to the fact that the bond is concave while the grain is convex.
...
\end{align*}
which is a Frèchet distribution! Thus in order to make assumptions on worst case scenario impacts that might effect missions to Europa, extreme value theory might be taken into consideration.
