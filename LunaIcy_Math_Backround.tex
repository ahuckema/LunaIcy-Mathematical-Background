...
As seen in Figure \ref{Fig: Sintering} there is a circle with radius $r_p$ that satisfies 
\[
...
The Surface area is in this model of crucial importance, as it is the exchange area of water vapor mass. For the grain we could use the complicated approach and calculate it from our derivation in the Appendix:
\[
S_g := r_g^2 \cdot \left( \pi -2 \int_0^{\text{arcos} (\frac{\hat{x}^*}{r_g})} \sqrt{1- \left( \frac{\hat{x}^*}{r_g \cos (\theta)} \right)^2}  d \theta  \right),
\]
but this is too complicated. A simple analytic solution is given by
\[
S_g (r_g) = 4\pi r_g^2 - 2 \pi r_g (r_g - \hat{x}^*) = 2 \pi r_g (r_g + \hat{x}^*).
\]
...
\]
where $\Phi$ is a Poisson point process on $\mathbb{R}^+ \times \mathcal{E}$ with intensity measure $\lambda \times \nu$, where $\lambda$ is the Lebesgue measure and $\nu$ a suitable measure on the space of Energy released by the meteor $\mathcal{E}$. The function $\mathcal{G}_i$ describes the change in the state vector due to an impact of a meteor with energy $E \in \mathcal{E}$. Due to the fact that an impact is assumed to be instantaneously, the resulting stochastic process $u(x,t)_{t \geq 0}$ is a jump process, i.e. Càdlàg, which forces us to use left limits in time. As long as there are no impacts, the stochastic Integral is zero and behaves deterministically. Note that the stochastic integral here over a Jump process is extremely tedious and the theory behind it is quite complicated. Maybe trying to find a stochastic process that is path wise a.s. continuous and approximates this Poisson point process, might make the derivation a little easier.\\
Further, this is a marked Poisson point process, where the marks are the energies at each impact of the meteor.\\
The density on the Energy space $\mathcal{E}$ is of the form 
\[
