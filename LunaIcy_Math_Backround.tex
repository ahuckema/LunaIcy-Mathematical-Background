...
\section{Outlook}
Consider the full state vector 
\[
u(x,t) := (T(x,t), m_g (x,t),m_b (x,t), \phi (x,t),\chi (x,t))^T,
\]
where $\phi(x,t)$ is the porosity and $\chi (x,t)$ the density of the ice-dust mixture. Then including micro meteor impacts the full stochastic PDE system is 
\begin{align*}
...
where 
\[
dI^{i}_{impacts} (x,t):= \int_{\mathcal{E}} \mathcal{G}_i( \lim_{\epsilon \to 0} u(x,t-\epsilon) ,E) \Phi (dt,dE), \quad i=1,2,3,4,
\]
where $\Phi$ is a Poisson point process on $\mathbb{R}^+ \times \mathcal{E}$ with intensity measure $\lambda \times \nu$, where $\lambda$ is the Lebesgue measure and $\nu$ a suitable measure on the space of meteor characteristics $\mathcal{E}$. The function $\mathcal{G}_i$ describes the change in the state vector due to an impact of a meteor with characteristics $E \in \mathcal{E}$. This system could be solved via splitting methods, where in each time step first the deterministic PDE system is solved and then the stochastic impact part is solved. This would require to simulate the Poisson point process and then update the state vector accordingly. This is left for future research. \\
...
\]
then satisfies by definition of the Poission distribution
\begin{align*}
...
\end{align*}
which is a Frèchet distribution! Thus in order to make assumptions on worst case scenario impacts that might effect missions to Europa, extreme value statistics needs be applied!
